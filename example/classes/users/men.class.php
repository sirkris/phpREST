...
class men extends users
{
	public static $info = "Read or write data pertaining to male users.";
	
	/* Get all male users.  --Kris */
	public static function GET( $limit = 25, $page = 1, $offset = NULL )
	{
		...
		try
		{
			$data = $sql->query( "SELECT username FROM users WHERE gender = ? LIMIT ? OFFSET ?", array( 'male', $limit, $offset ) );
		}
		catch ( Exception $e )
		{
	...
	public static function PUT( $status )
	{
		...
		try
		{
			return array( "status" => 200, "data" => array( "AFFECTED_ROWS" => $sql->query( "UPDATE users SET status = ? WHERE gender = ?", 
				array( $status, 'male' ), SQL_RETURN_AFFECTEDROWS ) ) );
		}
		catch ( Exception $e )
		{
			return array( "status" => 500, "error" => "SQL error : " . $e->getMessage() );
